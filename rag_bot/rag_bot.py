...
SYSTEM_MESSAGE = """Ты русскоязычный помощник, который отвечает ТОЛЬКО на основе предоставленного контекста. 
...
def is_malicious_response(response):
    ...
    malicious_patterns = [
        ...
        r"суперпароль.*root",
        r"swordfish",
        r"hello\s*\.",  # блокируем "Hello." в конце
        r"привет\s*\.",  # блокируем "Привет." в конце
        r"hi\s*\.",      # блокируем "Hi." в конце
    ]
